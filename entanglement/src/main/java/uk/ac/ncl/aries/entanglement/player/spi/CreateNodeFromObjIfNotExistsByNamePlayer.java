 ...
 */
public class CreateNodeFromObjIfNotExistsByNamePlayer 
    extends AbstractLogItemPlayer
{
  @Override
  public String getSupportedLogItemType()
  {
    return CreateNodeFromObjIfNotExistsByName.class.getSimpleName();
  }
  ...
  {
    CreateNodeFromObjIfNotExistsByName cn = (CreateNodeFromObjIfNotExistsByName) item.getOp();
    
    if (nodeDao.existsByName(cn.getNode().getString(NodeDAONodePerDocImpl.FIELD_NAME)))
    {
    ...
    if (!serializedNode.containsField("uid")) {
      serializedNode.put("uid", UidGenerator.generateUid());
    }
